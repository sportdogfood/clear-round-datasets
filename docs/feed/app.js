...
(function () {
  ...
  function resolveSaveUrl(boardJson) {
    // query param overrides (supports multiple names)
    const qp = qs('save_url') || qs('saveUrl') || qs('save') || qs('endpoint');
    if (qp && String(qp).trim()) return String(qp).trim();

    const meta = boardJson && (boardJson.meta || boardJson._meta || boardJson.metadata);
    const m = meta && (meta.save_url || meta.saveUrl || meta.endpoint || meta.patch_url);
  ...
  async function saveDraft(horseId) {
    ...
    if (!state.saveUrl) throw new Error('Save endpoint not configured.');
    ...
    // also provide flat keys (supports server expecting either)
    ...
    // If server returns board, consume it; otherwise re-GET
    let updated = null;
    try { updated = await res.json(); } catch (_) { updated = null; }

    if (updated) {
      let rows = null;
      if (Array.isArray(updated)) rows = updated;
      else if (Array.isArray(updated.rows)) rows = updated.rows;
      else if (Array.isArray(updated.board)) rows = updated.board;
      else if (Array.isArray(updated.data)) rows = updated.data;

      if (rows) {
        state.boardJson = updated;
        state.rows = rows;
        state.horses = deriveHorses(state.rows);
      } else {
        await loadBoard();
