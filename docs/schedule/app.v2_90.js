...
const URL_TRIPS    = urlCandidates('watch_trips.json');
  ...
  const start_nav_pro = document.getElementById('start_nav_pro');
  const start_nav_horses = document.getElementById('start_nav_horses');
  const start_toggle_details = document.getElementById('start_toggle_details');
  const start_toggle_label = document.getElementById('start_toggle_label');
  const start_details = document.getElementById('start_details');
  ...
  const state = {
    ...
    activeRing: '',     // '' or ring_number (Time view)
...
// ------------------------------------------------
  ...
  function setView(viewKey){
    ...
    // Filters:
    //  - Pro (lite): status + rings + grooms + horses
    //  - Full: rings + horses (NO status, NO grooms)
    //  - Time (summary): status + rings(filter) + grooms + horses
    const showFilters = (viewKey === 'lite' || viewKey === 'full' || viewKey === 'summary');
    const showStatus  = (viewKey === 'lite' || viewKey === 'summary');
    const showPeaks   = (viewKey === 'lite' || viewKey === 'full' || viewKey === 'summary');
    const showHorses  = (viewKey === 'lite' || viewKey === 'full' || viewKey === 'summary');
    const showGrooms  = (viewKey === 'lite' || viewKey === 'summary');

    statusWrap.hidden = !showStatus;
    peaksWrap.hidden  = !showPeaks;
    horsesWrap.hidden = !showHorses;
    if (groomsWrap) groomsWrap.hidden = !showGrooms;

    app.classList.toggle('filters--on', showFilters);
    app.classList.toggle('grooms--on', showFilters && showGrooms);
    app.classList.toggle('full-nostatus', viewKey === 'full');
  ...
  function buildHorseChips(){
    const isPro  = (state.activeView === 'lite');
    const isTime = (state.activeView === 'summary');
    const isFull = (state.activeView === 'full');

    // -------------------------
    // Groom chips (Pro + Time only)
    // Options ignore single-focus (activeHorse) and ignore activeGroom (so selector stays usable)
    // -------------------------
    if (groombar){
      groombar.innerHTML = '';
      if (isPro || isTime){
        const baseForGrooms = state.trips
          ...
          .filter(r => !isTime || !state.activeRing || String(r.ring_number||'') === String(state.activeRing || ''));

        const gset = new Set();
        baseForGrooms.forEach(r => {
          const g = (r.groomName ?? r.groom_name ?? r.groom ?? r.groomNameText ?? '').toString().trim();
          if (g) gset.add(g);
        });
        const grooms = Array.from(gset.values()).sort((a,b) => a.localeCompare(b));

        // If currently selected groom yields nothing under current filters, clear it
        if (state.activeGroom && !grooms.includes(state.activeGroom)) state.activeGroom = '';

        grooms.forEach(name => {
          const b = document.createElement('button');
          b.type = 'button';
          b.className = 'hchip' + ((state.activeGroom === name) ? ' is-on' : '');
          b.textContent = name;
    ...
    // -------------------------
    // Horse chips
    // Options ignore single-focus (activeHorse) but DO respect activeGroom / activeRing (Time)
    // -------------------------
    const baseForHorses = state.trips
      .filter(r => !isHorseInactive(r.horseName))
      .filter(r => !isPro && !isTime ? true : (!state.globalStatus || toStatusCode(r.latestStatus) === state.globalStatus))
      .filter(r => !isPro && !isTime ? true : (!state.activeGroom || ((r.groomName ?? r.groom_name ?? r.groom ?? r.groomNameText ?? '').toString().trim() === state.activeGroom)))
      .filter(r => !isTime || !state.activeRing || String(r.ring_number||'') === String(state.activeRing || ''));
  ...
  document.addEventListener('click', (e) => {
    ...
    // START: row taps + session details toggle (no new listeners)
    if (e.target.closest('#start_nav_pro')){
      setView('lite');
      return;
    }
    if (e.target.closest('#start_nav_horses')){
      setView('horses');
      return;
    }
    if (e.target.closest('#start_toggle_details')){
      if (start_details){
        const willShow = !!start_details.hidden;
        start_details.hidden = !willShow;
        if (start_toggle_label) start_toggle_label.textContent = willShow ? 'Close' : 'Open';
      }
      return;
    }
...
// ------------------------------------------------
  ...
  function renderPeaks(){
    ...
    // Base for ring pills:
    //  - Pro: apply inactive + status + activeGroom; ignore activeHorse
    //  - Time: apply inactive + status + activeGroom; ignore activeHorse and ignore activeRing for options
    //  - Full: derive from schedule (all rings that exist)
    let ringList = [];
    ...
    if (isFull){
      ...
      state.schedule.forEach(r => {
        ...
        if (!rset.has(key)) rset.set(key, { ring_number: rn, ringName: name || `Ring ${rn}` });
        else if (name && name.length > String(rset.get(key).ringName||'').length) rset.get(key).ringName = name;
      });
      ringList = Array.from(rset.values()).sort((a,b)=> (a.ring_number||0)-(b.ring_number||0));
    }else{
      const base = state.trips
        .filter(r => !isHorseInactive(r.horseName))
        .filter(r => !state.globalStatus || toStatusCode(r.latestStatus) === state.globalStatus)
        .filter(r => !state.activeGroom || ((r.groomName ?? r.groom_name ?? r.groom ?? r.groomNameText ?? '').toString().trim() === state.activeGroom));
    ...
    // Only show actionable rings (must exist in the currently visible body for the view)
    ringList.forEach((r) => {
      const rn = r.ring_number;
      const label = (r.ringName || (rn ? `Ring ${rn}` : 'Ring')).trim();

      if (isTime){
        // Ring FILTER (toggle)
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'peakbtn' + ((String(state.activeRing||'') === String(rn)) ? ' is-active' : '');
        btn.textContent = label;
        btn.addEventListener('click', () => {
          state.activeRing = (String(state.activeRing||'') === String(rn)) ? '' : String(rn);
          renderLiteAndFull();
        });
      ...
      // Ring ANCHOR (Pro/Full)
      const target = `#ring-${state.activeView}-${rn}`;
      const container = getActiveRingsContainer();
      const scope = container ? container.closest('.view') : null;
      const exists = scope ? !!scope.querySelector(target) : false;
      if (!exists) return;

      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'peakbtn';
      btn.textContent = label;
    ...
    // Default active state for anchor bars
    if (!isTime){
      const first = peakbar.querySelector('.peakbtn');
  ...
  function renderLite(){
    ...
    rings.forEach(rn => {
      ...
      ringClasses.forEach(c => {
        ...
        // (Full does not apply globalStatus filter)

        // entry filter (Pro ignores + optional single-horse focus)
        const entries = c.entries
          .filter(e => !isHorseInactive(e.horseName))
          .filter(e => !state.activeGroom || ((e.groomName ?? e.groom_name ?? e.groom ?? e.groomNameText ?? '').toString().trim() === state.activeGroom))
  ...
  function renderLiteAndFull(){
    ...
    // TIME (Summary) view: flattened Pro-style list (no ring grouping)
    if (time_container){
      if (state.activeView === 'summary'){
        // Build a single "ring card" styled header + list
        const classes = groupTripsToClasses()
          .slice()
          .sort((a,b) => {
            const ta = Number(a.time_sort||0), tb = Number(b.time_sort||0);
        ...
        const published = state.lastLoadedAt ? fmtWhen(state.lastLoadedAt) : '—';
        time_container.innerHTML = '';

        const wrap = document.createElement('section');
        wrap.className = 'ring_card';
        wrap.innerHTML = `
          <div class="ring_line">
            <div class="ring_title">Published ${esc(published)}</div>
            <div class="ring_actions" aria-hidden="true" style="visibility:hidden;"></div>
          </div>
          <div class="group_wrap"></div>
        `;

        const gw = wrap.querySelector('.group_wrap');
        ...
        classes.forEach(c => {
          ...
          if ((state.activeGroom || state.activeHorse) && entries.length === 0) return;
          ...
          classCard.innerHTML = `
            <div class="class_line" data-open-class="${esc(c.class_id)}" data-status="${statusCode}">
              ...
              <div class="c_tag">${esc(String(c.ring_number||''))}</div>
              <div class="c_badge"><div class="badge ${badgeClass(statusCode)}">${badgeInner(statusCode)}</div></div>
            </div>
            <div class="rollup_line"><div class="rollup_scroller"></div></div>
          `;

          const rollLine = classCard.querySelector('.rollup_scroller');
          entries.forEach(e => {
            const pill = document.createElement('div');
            pill.className = 'rollup_pill';
            pill.setAttribute('data-open-entry', String(e.entry_id || ''));
            pill.setAttribute('data-horse', String(e.horseName||'').trim());
            pill.innerHTML = rollPillInner(e);
        ...
        if (gw.children.length) time_container.appendChild(wrap);
      }else{
        time_container.innerHTML = '';
      }
    }
