...
// ------------------------------------------------
  ...
  function setView(viewKey){
    ...
    if (groombar) groombar.hidden = (viewKey === 'full');
    ...
    // re-render filters, because behavior differs by view
    buildHorseChips();
...
// ------------------------------------------------
  ...
  function renderPeaks(){
    ...
    rings.forEach((r, idx) => {
      ...
      // Time: ring pills act as a filter (not scroll anchors)
      if (state.activeView === 'summary'){
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'peakbtn' + ((state.activeRing === rn) ? ' is-active' : '');
      ...
      // Pro / Full: ring pills are scroll anchors
  ...
  function updateStartSummary(){
    ...
    const timeTitleEl = document.querySelector('#time_card .ring_title');
    if (timeTitleEl){
      timeTitleEl.textContent = state.lastLoadedAt ? (`Published ${fmtWhen(state.lastLoadedAt)}`) : 'Published —';
    }
  ...
  function renderLiteAndFull(){
    ...
    if (time_container){
      ...
      classes.forEach(c => {
        ...
        if (state.activeRing && Number(c.ring_number || 0) !== state.activeRing) return;
