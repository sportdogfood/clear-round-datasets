...
const URL_TRIPS    = urlCandidates('watch_trips.json');
  ...
  const state = {
    ...
    activeRing: 0,     // 0 or ring_number (Time ring filter)
...
// ------------------------------------------------
  ...
  function setView(viewKey){
    ...
    // Filters: Pro + Full + Time use horse/groom filters; Pro + Time use status; Pro/Full/Time use peaks
    ...
    // refresh filter bars on view switch
    buildHorseChips();
  ...
  if (startDetailsLink){
    startDetailsLink.addEventListener('click', (e) => {
      e.preventDefault();
      if (!startPanelData) return;
      const wasHidden = !!startPanelData.hidden;
      startPanelData.hidden = !wasHidden;
      startDetailsLink.textContent = wasHidden ? 'Session details (close)' : 'Session details';
    });
  }
  ...
  function buildHorseChips(){
    ...
    const baseForHorses = state.trips
      ...
      .filter(r => {
        if (state.activeView !== 'summary' || !state.activeRing) return true;
        return Number(r.ring_number || 0) === Number(state.activeRing);
      })
      .filter(r => {
        if (state.activeView === 'full' || !state.activeGroom) return true;
...
// ------------------------------------------------
  ...
  function renderPeaks(){
    ...
    rings.forEach((r, idx) => {
      ...
      btn.type = 'button';
      const isActive = (state.activeView === 'summary')
        ? (state.activeRing ? (state.activeRing === r.ring_number) : (idx===0))
        : (idx===0);
      btn.className = 'peakbtn' + (isActive ? ' is-active' : '');
      const label = (r.ringName || (r.ring_number ? `Ring ${r.ring_number}` : 'Ring')).trim();
      btn.textContent = label;
      btn.setAttribute('data-peak-target', target);
      btn.setAttribute('data-ring-number', String(r.ring_number || ''));
      btn.addEventListener('click', () => {
        Array.from(peakbar.querySelectorAll('.peakbtn')).forEach(x => x.classList.remove('is-active'));
        btn.classList.add('is-active');

        if (state.activeView === 'summary'){
          const rn = Number(btn.getAttribute('data-ring-number') || 0);
          state.activeRing = (state.activeRing === rn) ? 0 : rn;
          renderLiteAndFull();
          return;
        }
    ...
    rings.forEach(rn => {
      ...
      ringClasses.forEach(c => {
        ...
        const entries = c.entries
          ...
          .filter(e => (state.activeView === 'full' || !state.activeGroom) || (String(e.groomName || e.groom_name || e.groom || '').trim() === state.activeGroom))
  ...
  function renderFull(){
    ...
    Array.from(byRing.keys()).sort((a,b)=>a-b).forEach(rn => {
      ...
      ringRows.forEach(r => {
        ...
        // global status filter (NOT applied on Full)
        if (state.activeView !== 'full' && state.globalStatus && statusCode !== state.globalStatus) return;
  ...
  function renderLiteAndFull(){
    ...
    if (time_container){
      ...
      classes.forEach(c => {
        // filters: status + ring (Time)
        if (state.globalStatus && toStatusCode(c.latestStatus) !== state.globalStatus) return;
        if (state.activeRing && Number(c.ring_number||0) !== Number(state.activeRing)) return;

        // entry filter: inactive + groom/horse focus
        const entries = c.entries
          .filter(e => !isHorseInactive(e.horseName))
          .filter(e => !state.activeGroom || (String(e.groomName || e.groom_name || e.groom || '').trim() === state.activeGroom))
          .filter(e => !state.activeHorse || String(e.horseName||'').trim() === state.activeHorse);
