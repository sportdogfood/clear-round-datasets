/* tapactive-rings-v2.3 — app.v2_76.js
  ...
  document.addEventListener('click', (e) => {
    ...
    if (cls){
      ...
      const lines = [
        ...
        { c1:'', c2:(fmtStartShort(one.latestStart || '')), c3:(one.group_name || '—'), c4:minsTill, c5Html: nowHtml },
    ...
    if (pill){
      ...
      const smsBody2 = [
        `*** ${statusTxt.toUpperCase()} ***`,
        `${String(fmtGoShort(r.latestGO || ''))} | Ring ${String(r.ring_number ?? '—')} | #${String(r.class_number ?? '—')} ${String(r.class_name || '—')}`,
        `${String(r.horseName || '—')} (${String(entryNumber)}) | OOG ${String(r.lastOOG ?? '—')} | GO ${String(fmtGoShort(r.latestGO || ''))}`
      ].join('\n');
