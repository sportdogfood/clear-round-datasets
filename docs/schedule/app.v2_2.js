...
(function(){
  ...
  // Data paths (relative)
  // -----------------------------
  const URL_TRIPS    = '/docs/schedule/data/latest/watch_trips.json';
  const URL_SCHEDULE = '/docs/schedule/data/latest/watch_schedule.json';
  const URL_THREADS  = '/docs/schedule/data/latest/threads.json';
  ...
  async function loadAll(force=false){
    ...
    try{
      ...
      if (tr.status === 'fulfilled'){
        state.trips = Array.isArray(tr.value.records) ? tr.value.records : [];
      } else {
        state.trips = [];
      ...
      if (sc.status === 'fulfilled'){
        state.schedule = Array.isArray(sc.value.records) ? sc.value.records : [];
      } else {
        state.schedule = [];
      ...
      if (th.status === 'fulfilled'){
        state.threads = Array.isArray(th.value.records) ? th.value.records : [];
      } else {
        state.threads = [];
