/* tapactive-rings-v2.3 — app.v2_75.js
...
const URL_TRIPS    = urlCandidates('watch_trips.json');
  ...
  function statusIco(code){
    return code === 'U' ? icoClock() : code === 'L' ? icoBolt() : code === 'D' ? icoCheckCircle() : '';
  }
  ...
  function toStatusCode(latestStatus){
    ...
    if (s.includes('complete') || s.includes('done')) return 'D';
    return 'U';
  }
  ...
  function badgeClass(code){
    ...
    if (code === 'D') return 'badge--completed';
  ...
  function renderLite(){
    ...
    rings.forEach(rn => {
      ...
      ringClasses.forEach(c => {
        ...
        classCard.setAttribute('data-status', statusCode);
        classCard.setAttribute('data-class-id', String(c.class_id || ''));

        const timeTxt = fmtGoShort(c.latestStart || '—');
  ...
  function renderFull(){
    ...
    Array.from(byRing.keys()).sort((a,b)=>a-b).forEach(rn => {
      ...
      ringRows.forEach(r => {
        ...
        const timeTxt = fmtGoShort(r.latestStart || '—');
  ...
  document.addEventListener('click', (e) => {
    ...
    const ringAct = e.target.closest('[data-ring-action]');
    if (ringAct){
      e.preventDefault();
      e.stopPropagation();
      const a = ringAct.getAttribute('data-ring-action') || '';
      const map = { soon:'U', now:'L', done:'D' };
      const v = map[a] || '';
      state.globalStatus = (state.globalStatus === v) ? '' : v;
      document.querySelectorAll('[data-global-status]').forEach(b => {
        b.classList.toggle('is-on', (b.getAttribute('data-global-status') === state.globalStatus) && !!state.globalStatus);
...
openFly5(one.class_name || 'Class', lines, smsBody);
    ...
    if (pill){
      ...
      const stopDerives = (code === 'D') || (code === 'L' && lastGoneInVal === 1);
      ...
      const lines = [
        ...
        { c1:'', c2:riderNumber, c3:(r.riderName || '—'), c4:(r.lastOOG ?? '—'), c5:fmtGoShort(r.latestGO || '—') },
