/* tapactive-rings-v2.2 — app.v2_73.js
...
const URL_TRIPS    = urlCandidates('watch_trips.json');
    ...
    rings.forEach(rn => {
      ...
      ringClasses.forEach(c => {
        ...
        entries.forEach(e => {
          ...
          const hn = trunc6(e.horseName || '');
          const oog = ((e.lastOOG != null) && String(e.lastOOG) !== '') ? String(e.lastOOG) : '—';
          const go = fmtGoShort(e.latestGO || '');
          btn.innerHTML = `<span class="epill__name">${esc(hn)}</span><span class="epill__oog">• ${esc(oog)} •</span><span class="epill__time">${esc(go)}</span>`;
          sc.appendChild(btn);
        });
        // rollups start on the right and extend left
        try{ sc.scrollLeft = sc.scrollWidth; }catch(_e){}
  ...
  function openFly(title, rows, smsBody){
    ...
    (rows || []).forEach(r => {
      ...
      if (r && r.aHtml != null) c1.innerHTML = r.aHtml; else c1.textContent = a || ' ';
      if (r && r.bHtml != null) c2.innerHTML = r.bHtml; else c2.textContent = b || ' ';
      if (r && r.cHtml != null) c3.innerHTML = r.cHtml; else c3.textContent = c || ' ';
  ...
  document.addEventListener('click', (e) => {
    ...
    if (cls){
      ...
      // Type icon (fence vs horse) embedded in sequence cell (no new rows)
      const seq = String(one.schedule_sequencetype || '—');
      const ct = String(one.class_type || '');
      const isFlat = /saddle|flat/i.test(seq);
      const isJump = /jump/i.test(ct);
      const typeIcon = isFlat ? icoHorse() : (isJump ? icoFence() : '');

      const rows = [
        { label: 'Ring',  a: (one.ring_number ?? '—'),      b: (one.ringName || '—'),        c: '' },
        { label: 'Group', a: (one.class_group_id ?? '—'),   b: (one.group_name || '—'),      c: '' },
        { label: 'Class', aHtml: `${icoId()}<span style="margin-left:6px">${esc(one.class_number ?? '—')}</span>`, b: (one.class_name || '—'), c: '' },
        {               aHtml: `${typeIcon}${typeIcon ? '<span style="margin-left:6px">' + esc(seq) + '</span>' : esc(seq)}`, b: statusLabel(statusCode), c: '' },
        {               aHtml: `<span class="t-start">${esc(estStart)}</span>`, bHtml: `<span class="t-start">${esc(till)}</span>`, c: '' },
      ];
    ...
    if (pill){
      ...
      const entryNo = (r.backNumber != null && String(r.backNumber) !== '') ? r.backNumber
                    ...
                    : (r.entry_id != null && String(r.entry_id) !== '') ? r.entry_id
                    : '—';
      ...
      const lastGoneIn = (r.lastGoneIn != null && String(r.lastGoneIn) !== '') ? r.lastGoneIn : '—';
      const lastOOG = ((r.lastOOG != null) && String(r.lastOOG) !== '') ? r.lastOOG : '—';
      const latestGO = (r.latestGO != null && String(r.latestGO) !== '') ? r.latestGO : '—';
      const tillGO = (r.timetillgo != null && String(r.timetillgo) !== '') ? r.timetillgo : '—';
      const runningOOG = (running != null && String(running) !== '') ? running : '—';

      const rows = [
        { label: 'Ring',  a: (r.ring_number ?? '—'),     b: (r.ringName || '—'),     c: '' },
        { label: 'Entry', aHtml: `${icoId()}<span style="margin-left:6px">${esc(entryNo)}</span>`, b: (r.horseName || '—'),    c: '' },
        { label: 'Trip',  a: '',                        b: (r.riderName || '—'),    c: String(lastGoneIn) },
        {               a: '',                          b: String(lastOOG),         cHtml: `<span class="t-go">${esc(latestGO)}</span>` },
        {               a: '',                          b: String(runningOOG),      cHtml: `<span class="t-go">${esc(tillGO)}</span>` },
      ...
      const smsBody2 = [
        ...
        `${String(r.horseName || '—')} (${String(entryNo)}) | OOG ${String(runningOOG)} | GO ${String(latestGO)}`
      ].join('\n');

      openFly(r.horseName || 'Entry', rows, smsBody2);
      return;
    }
  });
