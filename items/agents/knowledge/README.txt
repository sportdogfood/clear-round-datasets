CRT AGENTS — KNOWLEDGE README (Session Boot & Rails) — v1.3

PURPOSE
This file is the fast, factual session boot + guardrails reference so every new session behaves the same. It complements (does not replace) the operator-facing guides.
...
- Ingestion rules: GET https://crt-b1434e13de34.herokuapp.com/items/agents/ingestion-rules.txt
...
- Commit endpoint: POST /items/commit
  Body:
    {
      "path": "items/index/.../file.ext",
      "json": { ... }  // for .json
      // or "json": "string body" for .txt/.md/.html/.js
      "message": "commit message"
    }
- Allowed file types: .json .txt .md .html .js
- Allowed dirs: governed by proxy ALLOW_DIRS (inspect via GET /items/_manifest.json)
...
STARTUP CHECKLIST (STOP ON ANY FAILURE; REPORT URL + STATUS)
- GET https://crt-b1434e13de34.herokuapp.com/health → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/agents/manifest.json → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/agents/ingestion-rules.txt → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/agents/event-rules.json → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/agents/venue-rules.json → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/agents/place-rules.json → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/agents/cards-core.txt → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/agents/labels-rules.json → 200
- GET https://crt-b1434e13de34.herokuapp.com/items/_manifest.json → 200
...
- labels are predefined only (items/index/labels/ one-file-per-label + _index.json)

VENUE GATE (APPLIES WHENEVER A VENUE BIND IS NEEDED)
- Provide venue_uid or venue_hint; resolve deterministically.
...
- Derive event_uid by stripping venue/organizer/sponsors/ratings/years/common terms (“horse show(s)”, “circuit”, “classic”, “series”, etc.) from the official title.
- Events use filename template: {organizer_uid}-{event_uid}.json (events only; filename_equals_uid=false).
- is_series ∈ {y|n|auto}. If series, legs sit fully inside hub window and do not overlap.
- Internal leg label = “Leg N”; public display must NOT say “Leg” unless the source does.

LABELS (GLOBAL TAGS)
- Store one file per label at items/index/labels/{label_uid}.json
- Use items/index/labels/_index.json for validation and synonyms mapping.
- Unknown labels: error for include/exclude; drop for auto-derived.
...
DIRECTORY SHAPES (CONFIRM VIA /items/_manifest.json)
...
AIRTABLE (OPTIONAL)
- Airtable is for collecting hints/exports only. The repo is the source of truth via /items/commit after PREVIEW.

SEE ALSO
- Operator README (deeper rationale & patterns): /items/agents/knowledge/OPERATOR-README.txt
- Task Runner README (commands overview):        /items/agents/README.txt
